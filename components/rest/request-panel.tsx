...
const PRE_REQ_SNIPPETS = [
  {
    ...
    code: `apilab.variables.set("variable_key", "variable_value");`,
  },
  {
    label: "Get an environment variable",
    code: `const value = apilab.variables.get("variable_key");`,
  },
  {
    label: "Clear a variable",
    code: `apilab.variables.set("variable_key", "");`,
  },
...
const POST_REQ_SNIPPETS = [
  {
    ...
    code: `if (apilab.response.status === 200) {\n  console.log("Success: Status is 200");\n} else {\n  console.log("Fail: Status is " + apilab.response.status);\n}`,
  },
  {
    label: "Get response body",
    code: `const body = apilab.response.body;\nconsole.log(body);`,
  },
  {
    label: "Get response header",
    code: `const contentType = apilab.response.headers["content-type"];`,
  },
  {
    label: "Check response time < 200ms",
    code: `if (apilab.response.time < 200) {\n  console.log("Response was fast!");\n}`,
  },
];
...
function RequestPanel() {
  ...
  const getMergedVariables = (currentTabVars: KeyValue[]) => {
    ...
    store.globalVariables.forEach((v) => {
  ...
  const handleSend = async () => {
    ...
    const createSandbox = (vars: KeyValue[], response: any = null) => ({
      variables: {
        get: (key: string) =>
          vars.find((v) => v.key === key && v.enabled)?.value,
    ...
    try {
      const headerObj = headers.reduce((acc, curr) => {
        if (curr.enabled && curr.key) {
          acc[replaceVariables(curr.key, variableMap)] = replaceVariables(
            curr.value,
            variableMap
          );
        }
      ...
      if (
        ...
        headerObj["Authorization"] = `Bearer ${replaceVariables(
          auth.token,
          variableMap
        )}`;
      ...
      if (reqMode === "proxy") {
        ...
        Object.entries(paramObj).forEach(([k, v]) =>
          full.searchParams.set(k, v as string)
        );
      ...
      if (activeTab.postRequestScript) {
        ...
        runScript(
          ...
          createSandbox(currentTabVars, scriptResponse)
        );
        store.setVariables(currentTabVars);
      }
      ...
      if (session) {
        ...
        const existing = JSON.parse(
          localStorage.getItem("apilab_history") || "[]"
        );
    ...
    <div className="flex flex-col h-full min-h-0 bg-background">
      ...
      <div className="block md:flex gap-2 space-y-2 md:space-y-0 p-2  items-center flex-col md:flex-row">
        <div className="flex gap-2 md:w-[90%]">
          ...
          <Input
            ...
            value={replaceVariables(
              activeTab.url,
              activeTab.variables.reduce((acc, curr) => {
        ...
        <div className="flex gap-2">
          <Select
            value={reqMode}
            onValueChange={(v: any) => store.setReqMode(v)}
          >
      ...
      <Tabs defaultValue="params" className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-1 min-h-0">
          <TabsContent value="params" className="mt-0 h-full tab-fade">
            <KeyValueTable
              items={queryParams}
              setItems={store.setQueryParams}
            />
          </TabsContent>
          <TabsContent value="headers" className="mt-0 h-full tab-fade">
            <KeyValueTable
              items={headers}
              setItems={store.setHeaders}
              isHeader={true}
            />
          </TabsContent>
          <TabsContent
            value="body"
            className="mt-0 h-full flex flex-col gap-2 tab-fade"
          >
            <ReqBody />
          </TabsContent>
          ...
          <TabsContent value="vars" className="mt-0 h-full tab-fade">
            <div className="p-2 h-full flex flex-col">
              <p className="text-xs text-muted-foreground p-2">
                Request variables (Local to this request). Use in URL/Headers as{" "}
                <code className="bg-muted px-1 rounded">{"{{key}}"}</code>
              </p>
              <div className="flex-1 min-h-0">
                <KeyValueTable
                  items={variables}
                  setItems={store.setVariables}
                />
              </div>
            </div>
          </TabsContent>
