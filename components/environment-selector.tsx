...
useEffect(() => {
  ...
  const handleEdit = (e: React.MouseEvent, env: Environment) => {
    ...
    setEditingEnv({ ...env, variables: (env.variables || []).map(v => ({...v})) });
    setOpenEditor(true);
  };
    ...
    <div className="flex items-center gap-1 border-l pl-2 ml-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[250px]">
          ...
          <DropdownMenuItem className="text-xs gap-2 cursor-pointer" onClick={() => store.setActiveEnvId(null)}>
            <div className="w-4 flex justify-center">{!activeEnv && <Check size={12} />}</div>
            No environment
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          {store.environments.map(env => (
             <DropdownMenuItem key={env._id} className="text-xs gap-2 cursor-pointer group" onClick={() => store.setActiveEnvId(env._id)}>
                <div className="w-4 flex justify-center">{activeEnv?._id === env._id && <Check size={12} />}</div>
                <span className="flex-1 truncate">{env.name}</span>
                <Pencil size={12} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-foreground" onClick={(e) => handleEdit(e, env)} />
             </DropdownMenuItem>
          ))}
          <DropdownMenuSeparator />
      ...
      <Dialog open={openEditor} onOpenChange={setOpenEditor}>
        <DialogContent className="sm:max-w-2xl h-[60vh] flex flex-col">
            <DialogHeader><DialogTitle>{editingEnv._id ? "Edit Environment" : "Create Environment"}</DialogTitle></DialogHeader>
            <div className="flex flex-col gap-4 flex-1 min-h-0 py-2">
                <Input value={editingEnv.name} onChange={(e) => setEditingEnv(prev => ({...prev, name: e.target.value}))} placeholder="Environment Name" />
                <div className="flex-1 border rounded-md overflow-hidden relative">
                    <div className="absolute inset-0 overflow-auto">
                        <KeyValueTable items={editingEnv.variables || []} setItems={(vars) => setEditingEnv(prev => ({...prev, variables: vars}))} />
                    </div>
                </div>
            </div>
            <DialogFooter className="flex justify-between! w-full sm:justify-between">
                {editingEnv._id ? <Button variant="destructive" size="sm" onClick={deleteEnvironment}><Trash2 size={14} className="mr-2"/> Delete</Button> : <div/>}
